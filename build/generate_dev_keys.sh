...
openssl genrsa -des3 -passout pass:password324 -out server-dev.pass.key 2048
openssl rsa -passin pass:password324 -in server-dev.pass.key -out server-dev.key
rm server-dev.pass.key
...
openssl x509 -req -sha256 -days 365 -in server-dev.csr -signkey server-dev.key -out server-dev.crt
...
#certutil -encode server-dev.key encodedkey.txt
base64 server-dev.key
...
echo "Certificate and key generation complete, please add server-dev.crt to your OAuth connected app in Salesforce"